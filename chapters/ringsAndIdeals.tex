...
\begin{proof}
    \begin{enumerate}
        \item 
            ...
            A set of generators of $(\langle T \rangle R)S$ over $S$ is $\{f(\sum_i^{\text{finite}}t_ir_i) = \sum_i^{\text{finite}} f(t_i)f(r_i) \mid t_i \in T,\ r_i \in S, \fa i\} \subseteq \langle f(T) \rangle S$. A set of generators of $\langle f(T) \rangle S$ over $S$ is $\{f(t) \mid t \in T\} = \{f(t \cdot 1) \mid t \in T\}$ which is a subset of the generators of $(\langle T \rangle R)S$.
        \item 
