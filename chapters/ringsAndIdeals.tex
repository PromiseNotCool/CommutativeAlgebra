...
\begin{fact}\label{1.4}
    ...
    \begin{enumerate}
        ...
        \item $\phi(\sum_{i=1}^mr_is_i) = \sum_{i=1}^m\phi(r_i)\phi(s_i)$ for $r_1,\ldots,r_m,s_1,\ldots,s_m \in R$.
...
\begin{fact}[Subring test]\label{1.6}
...
\begin{fact}[Ideal test]\label{1.10}
...
\begin{example}\label{1.11}
    \begin{enumerate}
        ...
        \item 
            If $r_1,\ldots,r_m \in R$, then 
            \begin{align*}
                \langle r_1,\ldots,r_m \rangle &= \langle r_1,\ldots,r_m \rangle R = (r_1,\ldots,r_m) = (r_1,\ldots,r_m)R = \bigcap_{r_1,\ldots,r_m \in I \leq R} I \\
                                               &= \left\{\sum_{i=1}^m a_ir_i \mathrel{\bigg |} a_i \in R, \fa i = 1,\ldots,m\right\} \leq R. 
            \end{align*}
            In particular,  
            \[\langle r \rangle = \langle r \rangle R = (r) = (r)R = rR = Rr = \{ar \mid a \in R\} = \bigcap_{r \in I \leq R}I, \fa r \in R.\]
        \item 
...
\begin{fact}\label{1.12}
    For any $r_1,\ldots,r_m \in R$, $\langle r_1,\ldots,r_m \rangle$ is the smallest ideal of $R$ containing $r_1,\ldots,r_m$, i.e., for any $\ffa \leq R$, $r_1,\ldots,r_m \in \ffa$ if and only if $\langle r_1,\ldots,r_m \rangle \subseteq \ffa$. Similarly, $A \subseteq \ffa$ if and only if $\langle A \rangle \subseteq \ffa$, e.g., if $A \leq R$, then $A = \langle A \rangle$.
\end{fact}

\begin{construction}\label{1.13}
...
\begin{fact}[Ideal correspondence for quotients]\label{1.15}
    ...
    \begin{center}
        \begin{tikzcd}
            R \ar[rr,"p"] \ar[ddd,swap,"\pi"] &[-32pt] & R/\ffa \ar[rr,"\tau"] & &[+15pt] \frac{R/\ffa}{J/\ffa} \\ [-25pt]
            & r \ar[r,mapsto] \ar[d,mapsto] & \overbar{r} \ar[r,mapsto] & \overbar{\overbar{r}} \\
            & \overbar{r} \ar[rru,mapsto] \\ [-19pt] 
...
\begin{notation*}
    $\operatorname{Spec}(R) = \{\text{primes ideals of $R$}\}$, called the \emph{prime spectrum of} $R$. \par
    The variety determined by an ideal $\ffa \leq R$ is $\operatorname{V}(\ffa) = \{\ffp \in \operatorname{Spec}(R) \mid \ffp \supseteq \ffa\}$. \par
    $\operatorname{m-Spec}(R) = \{\text{maximal ideals of $R$}\} \subseteq \operatorname{Spec}(R)$.
\end{notation*}

\begin{fact}\label{1.16}
    ...
    \begin{center}
        \begin{tikzcd}
            R \ar[rr,"\phi"] \ar[ddd,swap,"p"] &[-37pt] & S \ar[rr,"\pi"] & &[+10pt] S/\ffq \\ [-25pt]
...
\begin{example}\label{1.19}
    \begin{enumerate}
        ...
        \item Let $k$ be a field and $R = k[X_1,\ldots,X_d]/\langle X_1^{a_1}, \cdots, X_d^{a_d}\rangle$, where $a_i \geq 1$ for $i = 1,\ldots,d$. Then $R$ is local with $\ffm = \langle X_1,\ldots,X_d\rangle/\langle X_1^{a_1},\ldots,X_d^{a_d} \rangle$. In fact, $\operatorname{Spec}(R) = \left\{\langle X_1,\ldots,X_d\rangle/\langle X_1^{a_1}, \cdots, X_d^{a_d}\rangle\right\}$.
    \end{enumerate}
\end{example}
...
\begin{example}\label{1.25}
    In the ring $\bbZ/\langle p^n \rangle$, we have $\overbar p$ is nilpotent. It is similar in $k[X]/\langle X^n \rangle$ and $k[X_1,\ldots,X_n]/\langle X_1^{a_1},\ldots,X_d^{a_d}\rangle$, where $k$ is a field, $n \geq 1$ and $a_1\cdots,a_d \geq 1$.
\end{example}
...
\begin{proof}
    \begin{enumerate}
        \item Since $0 \in \operatorname{Nil}(R)$, $\operatorname{Nil}(R) \neq \emptyset$. Let $r \in R$ and $a,b \in \operatorname{Nil}(R)$. Then there exists $m,n \geq 1$ such that $a^m = 0 = b^n$. Then $(ra)^m = r^ma^m = 0$ and so $ra \in \operatorname{Nil}(R)$. By the binomial theorem, $(a+b)^{m+n} = \sum_{i=0}^{m+n} \binom {m+n} i a^i b^{m+n-i} = 0$. Since for $i = 0,\ldots,m+n$, either $i \geq m$ or $i < m$, i.e., $i \geq m$ or $m+n-i > n$, we have $a^i = 0$ when $i \geq m$, and $b^{m+n-i} = 0 $ when $m + n-i > n$. So $(a+b)^{m+n} = 0$ and thus $a+b \in \operatorname{Nil}(R)$.
...
\begin{example*}
    Let $k$ be a field and $R = k[X_1,\ldots,X_d]/\langle X_1^{a_1},\ldots,X_d^{a_d} \rangle \neq 0$, where $a_i \geq 1$ for $i = 1,\ldots,d$. Then $\operatorname{Nil}(R) = \langle X_1,\ldots,X_d \rangle / \langle X_1^{a_1},\ldots,X_d^{a_d} \rangle$.
\end{example*}

\begin{proof}
    Method 1. Since $\operatorname{Spec}(R) = \left\{\langle X_1,\ldots,X_d \rangle / \langle X_1^{a_1},\ldots,X_d^{a_d} \rangle\right\}$, $\operatorname{Nil}(R) = \bigcap_{\ffp \in \operatorname{Spec}(R)} \ffp =  \langle X_1,\ldots,X_d \rangle / \langle X_1^{a_1},\ldots,X_d^{a_d} \rangle$. \par
    Method 2. Since $\overbar {X_i} \in \operatorname{Nil}(R) \leq R$ for $i = 1,\ldots,d$, we have $\overbar {\langle X_1,\ldots,X_d \rangle} = \langle \overbar {X_1},\ldots,\overbar {X_d} \rangle \subseteq \operatorname{Nil}(R) \subsetneq R$ since $R \neq 0$. Also, since $\overbar {\langle X_1,\ldots,X_d \rangle}$ is maximal, we have $\operatorname{Nil}(R) = \overbar {\langle X_1,\ldots,X_d \rangle}$.
\end{proof}

\begin{fact*}
    If $\ffa \leq R$ and $r_1,\ldots,r_n \in R$, then $R/\ffa \supseteq \langle \overbar r_1,\ldots,\overbar r_n \rangle = \langle r_1,\ldots,r_n, \ffa \rangle / \ffa$. In particular, if $\langle r_1,\ldots,r_n \rangle \supseteq \ffa$, then $\langle \overbar r_1, \cdots, \overbar r_n \rangle = \langle r_1,\ldots,r_n \rangle / \ffa$.
\end{fact*}
...
Let $\ffa,\ffb,\ffc \leq R$, $\ffa_1,\ldots,\ffa_n \leq R$, $S_\lambda \subseteq R$ and $\ffa_\lambda,\ffb_\lambda \leq R$ for $\lambda \in \Lambda$, where $\Lambda$ is an index set.
...
\begin{fact}\label{1.31}
    ...
    \begin{enumerate}
        ...
        \item \label{1.31e} If $\ffa = \langle x_1,\ldots,x_m \rangle$ and $\ffb = \langle y_1,\ldots,y_n \rangle$, then $\ffa + \ffb = \langle x_1,\ldots,x_m, y_1,\ldots,y_n \rangle$.
        \item \label{1.31f} If $x \in R$, then $\langle x,\ffa \rangle = \langle x \rangle + \ffa$.
        \item \label{1.31g} $\ffa + (\ffb + \ffc) = (\ffa + \ffb) + \ffc$.
    \end{enumerate}
\end{fact}
...
\begin{remark}
    The sum $\ffa_1 + \cdots + \ffa_n$ is defined for $\ffa_1,\ldots,\ffa_n$ for all $n \in \bbZ_{\geq 3}$ and same properties as above hold for finite sums.
\end{remark}
...
\begin{fact}\label{1.38}
    ...
    \begin{enumerate}
        ...
        \item\label{1.38e} If $\ffa = \langle x_1,\ldots,x_m \rangle$ and $\ffb = \langle y_1,\ldots,y_n \rangle$, then $\ffa\ffb = \langle x_iy_j \mid i = 1,\ldots,m,j = 1,\ldots,n \rangle$.
...
\begin{proof}
    \begin{enumerate}
        ...
        \item [(f)]
            Method 1. For any $a \in \ffa \leq R$, we have $ab \in \ffa$ for any $b \in \ffb$. For any $b \in \ffb \leq R$, we have $ab \in \ffb$ for any $a \in \ffa$. So $ab \in \ffa \cap \ffb$ for any $a \in \ffa$ and $b \in \ffb$. Hence $\ffa \ffb \subseteq \ffa \cap \ffb$ by Fact \ref{1.12}. \par
            Method 2. It follows from $\ffa \ffb \subseteq \ffa R = \ffa$ and $\ffa\ffb \subseteq R\ffb = \ffb$.
            \qedhere
    \end{enumerate}
\end{proof}
...
\begin{fact}\label{1.44}
    Let $n \geq 1$ and $N = \{a_1 \cdots a_n \mid a_i \in \ffa, \fa i = 1,\ldots,n\}$.
    \begin{enumerate}
        \item
        ...
        \item $\ffa^n = \{\sum_{i}^{\text{finite}}a_{i1} \cdots a_{in} \mid a_{ij} \in \ffa, \fa i, \fa j = 1,\ldots,n\}$.
        \item If $\ffa = \langle S \rangle$, then $\ffa^n = \langle s_1 \cdots s_n \mid s_i \in S, \fa i = 1,\ldots,n\rangle$.
        \item 
            If $\ffa = \langle x_1,\ldots,x_m \rangle$, then $\ffa^n = \langle x_{i_1} \cdots x_{i_n} \mid i_j \in \{1,\ldots,m\}, \fa j = 1,\ldots,n \rangle$.
    \end{enumerate}
\end{fact}

\begin{fact}\label{1.45}
...
\begin{proposition}[Chinese Remainder Theorem]\label{1.46}
    We have the following.
    \begin{enumerate}
        \item\label{1.46a} The function $\phi: R \to (R/\ffa_1) \times \cdots \times (R/\ffa_n)$ given by $\phi(x) = (\overbar x, \cdots, \overbar x) = (x+\ffa_1,\ldots,x+\ffa_n)$ is a well-defined ring homomorphism.
        \item\label{1.46b} If $\ffa_i + \ffa_j = R$ for $1 \leq i,j \leq n$ with $i \neq j$, i.e., $\{\ffa_1,\ldots,\ffa_n\}$ are pairwise coprime, then $\bigcap_{i=1}^n \ffa_i = \ffa_1 \cdots \ffa_n$ and $\ffa_i + (\bigcap_{j=1,j \neq i}^{n} \ffa_j)R = R$ for $i = 1,\ldots,n$.
...
\begin{proof}
    \begin{enumerate}
        ...
        \item [(c)]
            ``$\Rightarrow$''. Assume $\phi$ is surjective. In particular, there exists $x \in R$ such that $(\overbar 1,\overbar 0,\ldots,\overbar 0) = \phi(x) = (\overbar x, \overbar x, \cdots, \overbar x)$. So $x+\ffa_1 = 1+\ffa_1$ and $x+\ffa_i = 0 + \ffa_i$ for $i = 2,\ldots,n$. Hence $1-x \in \ffa_1$ and $x \in \ffa_i$ for $i = 2,\ldots,n$. Also, since $\mathpunct{\underset{\mathclap{\substack{\ \in \ffa_i \\ \textstyle }}}{(x)}} + \mathpunct{\underset{\mathclap{\substack{\ \in \ffa_1 \\ \textstyle }}}{(1-x)}} = 1$, we have $\ffa_i + \ffa_1 = R$ for $i = 2,\ldots, n$. Similarly, consider $(\overbar 0, \cdots, \overbar 0, \mathpunct{\underset{\mathclap{\substack{\ \ \ \uparrow j\text{th} \\ \textstyle }}}{\overbar 1}}, \overbar 0, \cdots, \overbar 0) \longsquiggly \ffa_i + \ffa_j = R$ for $1 \leq i,j \leq n$ with $i \neq j$. \par 
            ``$\Leftarrow$''. Assume $\ffa_i + \ffa_j = R$ for $1 \leq i,j \leq n$ with $i \neq j$. By (b), $\ffa_1 + (\bigcap_{j=2}^n \ffa_j)R = R$. So $a_1 + y = 1$ with $a_1 \in \ffa_1$ and $y \in \bigcap_{j=2}^n \ffa_j$, i.e., $1-y = a_1 \in \ffa_1$ and $y \in \ffa_j$ for $j = 2,\ldots, n$. Then $\phi(y) = (\overbar y, \overbar y, \cdots, \overbar y) = (y + \ffa_1, y + \ffa_2, \cdots, y + \ffa_n) = (1+\ffa_1,0+\ffa_2,\ldots,0+\ffa_n) = (\overbar 1,\overbar 0, \cdots, \overbar 0)$. Similarly, for $j = 1,\ldots,n$, there exists $y_j$ such that $\phi(y_j) = (\overbar 0, \cdots, \overbar 0, \mathpunct{\underset{\mathclap{\substack{\ \ \ \uparrow j\text{th}\\ \textstyle }}}{\overbar 1}}, \overbar 0, \cdots, \overbar 0)$. Then for any $(\overbar r_1,\ldots, \overbar r_n) \in \frac{R}{\ffa_1} \times \cdots \times \frac{R}{\ffa_n}$, $(\overbar r_1,\ldots, \overbar r_n) = \sum_{j=1}^n r_j(\overbar 0, \cdots, \overbar 0, \mathpunct{\underset{\mathclap{\substack{\ \ \ \uparrow j\text{th} \\ \textstyle }}}{\overbar 1}}, \overbar 0, \cdots, \overbar 0) = \sum_{j=1}^n r_j\phi(y_j) = \phi(\sum_{j=1}^nr_jy_j)$. So $\phi$ is surjective. \qedhere
    \end{enumerate}
\end{proof}

\begin{proposition}\label{1.47}
    Let $\ffa_1,\ldots,\ffa_n \leq R$ and $\ffp \in \operatorname{Spec}(R)$.
    \begin{enumerate}
        \item\label{1.47a} If $\ffp = \ffa_1 \cdots \ffa_n$, then $\ffp = \ffa_i$ for some $i \in \{1,\dots,n\}$.
        \item\label{1.47b} If $\ffp \supseteq \ffa_1 \cap \cdots \cap \ffa_n$, then $\ffp \supseteq \ffa_i$ for some $i \in \{1,\ldots,n\}$.
        \item\label{1.47c} If $\ffp = \ffa_1 \cap \cdots \cap \ffa_n$, then $\ffp = \ffa_i$ for some $i \in \{1,\ldots,n\}$.
    \end{enumerate}
\end{proposition}

\begin{proof}
    \begin{enumerate}
        \item [(b)] Assume $\ffp \supseteq \ffa_1 \cap \cdots \cap \ffa_n \supseteq \ffa_1 \cdots \ffa_n$ by Fact~\ref{1.38}\ref{1.38f}. Since $\ffp \in \operatorname{Spec}(R)$, there exists some $i \in \{1,\ldots,n\}$ such that $\ffp \supseteq \ffa_i$. 
        \item[(c)]
            By (b), there exists $i \in \{1,\ldots,n\}$ such that $\ffa_i \subseteq \ffp = \ffa_1 \cap \cdots \cap \ffa_n \subseteq \ffa_i$. So $\ffp = \ffa_i$. 
        \item[(a)] Since $\ffp \supseteq \ffa_1 \cdots \ffa_n$, we have $\ffp \supseteq \ffa_i$ for some $i \in \{1,\dots,n\}$. Also, we have $\ffp = \ffa_1 \cdots \ffa_n \subseteq \ffa_i$. \qedhere
    \end{enumerate}
\end{proof}
...
\begin{example}\label{1.49}
    ...
    The same technique shows that can't replace $V_1,\ldots,V_n$ with $V_1,V_2,\cdots$ over $\bbQ$.
\end{example}

\begin{theorem}[Prime avoidence, general version]\label{1.50}
    Let $\ffb_1,\ldots,\ffb_n, \ffa \leq R$. Assume
    \begin{enumerate}
        \item $R$ contains an infinite field $k$ as a subring, or
        \item $\ffb_3,\ldots,\ffb_n \in \operatorname{Spec}(R)$.
    \end{enumerate}
    Then if $\ffa \not \subseteq \ffb_i$ for all $i = 1,\ldots,n$, then $\ffa \not \subseteq \bigcup_{i=1}^n \ffb_i$.
\end{theorem}

\begin{proof}
    \begin{enumerate}
        \item For each $i = 1,\ldots,n$, since $\ffa \not \subseteq \ffb_i$, $\ffa \cap \ffb_i \lneq \ffa$. Also, since $\ffa$ is a $k$-vector space, by Lemma \ref{1.48}, $\ffa \cap \bigcup_{i=1}^n \ffb_i = \bigcup_{i=1}^n (\ffa \cap \ffb_i) \lneq \ffa$. So $\ffa \not \subseteq \bigcup_{i=1}^n \ffb_i$. 
        \item Induct on $n$. Base case $n = 1$: done. Base case $n = 2$. Let $a_i \in \ffa \smallsetminus \ffb_i$ for $i = 1,2$. Then $a_1 + a_2 \in \ffa$. Suppose $\ffa \subseteq \ffb_1 \cup \ffb_2$. Then $a_1 + a_2 \in \ffb_1 \cup \ffb_2$, say $a_1 + a_2 \in \ffb_2$. Since $a_1 \in \ffa \subseteq \ffb_1 \cup \ffb_2$ and $a_1 \not \in \ffb_1$, $a_1 \in \ffb_2$. So $a_2 = (a_1 + a_2) - a_1 \in \ffb_2$, a contradiction. \par 
            Induction step $n \geq 3$. Let $\ffa \not \subseteq \ffb_i$ for $i = 1,\ldots,n$. Assume $\ffa \not \subseteq \bigcup_{i \neq j} \ffb_i$ for $j = 1,\ldots,n$. Then there exists $a_j \in \ffa \smallsetminus \{\bigcup_{i \neq j} \ffb_i\}$ for $j = 1,\ldots,n$. By way of contradiction, suppose $\ffa \subseteq \bigcup_{i=1}^n \ffb_i$. Then $a_j \in \bigcup_{i=1}^n \ffb_i \smallsetminus \{\bigcup_{i \neq j}\ffb_i\} \subseteq \ffb_j$ for $j = 1,\ldots,n$. Note $a_1 \cdots a_{n-1} + a_n \in \ffa \subseteq \bigcup_{i=1}^n \ffb_i$. So there exists $l \in \{1,\ldots,n\}$ such that $a_1 \cdots a_{n-1} + a_n \in \ffb_l$. Suppose $l=n$. Since $a_n \in \ffb_n$, $a_1 \cdots a_{n-1} \in \ffb_n$. Since $n \geq 3$, we have $\ffb_n \in \operatorname{Spec}(R)$ and then $a_i \in \ffb_n$ for some $1 < i < n$, a contradiction. Hence we must have $l < n$. But since $a_1 \cdots a_l \cdots a_{n-1} \in \ffb_l$, we have $a_n \in \ffb_l$, a contradiction. \qedhere
    \end{enumerate}
\end{proof}

\begin{theorem}[Prime avoidence]\label{1.51}
    Let $\ffp_1,\ldots,\ffp_n \in \operatorname{Spec}(R)$. If $\ffa \subseteq \bigcup_{i=1}^n \ffp_i$, then $\ffa \subseteq \ffp_i$ for some $i \in \{1,\ldots,n\}$, i.e., if $\ffa \not \subseteq \ffp_i$ for $i = 1,\ldots,r$, then $\ffa \not \subseteq \bigcup_{i=1}^n \ffp_i$.
\end{theorem}

\begin{fact*}[Avoidence for monomial ideals] 
    Let $A$ be a nonzero commutative ring with identity and $\ffa,\ffb_1,\ldots,\ffb_n$ be monomial ideals of $A[X_1,\ldots,X_d]$. If $\ffa \subseteq \bigcup_{i=1}^n \ffb_i$, $\ffa \subseteq \ffb_i$ for some $i \in \{1,\ldots,n\}$.
\end{fact*}

\begin{proof}
     By Dickson's lemma, $\ffa = \langle f_1,\ldots,f_m \rangle$ for some monomials $f_1,\ldots,f_m \in A[X_1,\ldots,X_d]$. Then $f_1 + \cdots + f_m \in \ffa \subseteq \bigcup_{i=1}^n \ffb_i$. So $f_1 + \cdots + f_m \in \ffb_i$ for some $i \in \{1,\ldots,n\}$. But $\ffb_i$ is a monomial ideal, so $f_1,\ldots,f_m \in \ffb_i$. Thus, $\ffa = \langle f_1,\ldots,f_m \rangle \subseteq \ffb_i$.
\end{proof}
...
\begin{fact}\label{1.58}
    ...
    \begin{enumerate}
        ...
        \item\label{1.58h} $\operatorname{rad}(\bigcap_{i=1}^n \ffp_i^{e_i}) = \bigcap_{i=1}^n \ffp_i$, where $\ffp_i \in \operatorname{Spec}(R)$ and $e_i \geq 1$ for $i = 1,\ldots,n$.
...
\begin{example}\label{1.59}
    \begin{enumerate}
        ...
        \item[(d)] Example of $\operatorname{rad}(\bigcap_{i=1}^\infty \ffa_i) \subsetneq \bigcap_{i=1}^\infty \operatorname{rad}(\ffa_i)$. Let $R = k[X_1,X_2,\cdots]$, $\ffa_1 = \langle X_1 \rangle$, $\ffa_2 = \langle X_1^2,X_2^2 \rangle$, $\cdots$, $\ffa_i = \langle X_1^i,\ldots,X_i^i \rangle$, $\cdots$. Since $\langle X_1,\ldots,X_i \rangle \in \operatorname{Spec}(R)$ for $i \geq 1$, by (f) and (g), we have $\operatorname{rad}(\ffa_i) = \operatorname{rad}(\langle X_1^i,\ldots,X_i^i \rangle) = \operatorname{rad}(\langle X_1,\ldots,X_i \rangle) = \langle X_1,\ldots,X_i \rangle$ for $i \geq 1$. So $\bigcap_{i=1}^\infty \operatorname{rad}(\ffa_i) = \bigcap_{i=1}^\infty \langle X_1,\ldots,X_i \rangle = \langle X_1 \rangle \supsetneq 0 = \operatorname{rad}(0) = \operatorname{rad}(\bigcap_{i=1}^\infty \ffa_i)$. 
...
\begin{proof}
    \begin{enumerate}
        ...
        \item 
            ...
            \end{align*}
        \item 
            Let $s \in \operatorname{rad}(\ffa)S$. Then there exist $m \geq 1$, $a_i \in \operatorname{rad}(\ffa)$ and $s_i \in S$ for $i = 1,\ldots,m$ such that $s = \sum_{i=1}^m f(a_i)s_i$. Since $a_i \in \operatorname{rad}(\ffa)$, there exists $n_i \geq 1$ such that $a_i^{n_i} \in \ffa$ for $i = 1,\ldots,m$. Let $n = n_1 + \cdots + n_m$. Note if $k_1 + \cdots + k_m = n$ with $k_1,\ldots,k_m \geq 0$, then there exists some $i \in \{1,\ldots,m\}$ such that $k_i \geq n_i$ and so $a_i^{k_i} \in \ffa$. Hence  
...
\begin{proposition}\label{1.68}
    ...
    \begin{enumerate}
        ...
        \item Let $\ffm \in \operatorname{m-Spec}(A\llbracket X \rrbracket)$. Then 
            \begin{enumerate}
...
\begin{proof}
    \begin{enumerate}
        ...
        \item 
            ...
            In addition, assume $\ffa = (\alpha_1,\ldots,\alpha_n)A$ for some $\alpha_1,\ldots,\alpha_n \in \ffa$. Let $f \in \ffa \llbracket X \rrbracket$. Then $a_i \in \ffa = (\alpha_1,\ldots,\alpha_n)A$ for $i \geq 0$. So for $i \geq 0$, we have $a_i = \sum_{j=1}^n b_{ij}\alpha_j$ for some $b_{i1},\ldots,b_{in} \in A$. Hence by the definition of addition and multiplication in $A\llbracket X \rrbracket$, 
        ...
        \item 
            ...
            Assume $a_i \in \operatorname{Nil}(A)$ for $i \geq 0$ and $\langle a_0,a_1,\cdots \rangle$ is finitely generated. Then $\langle a_0,a_1,\cdots \rangle = \langle a_0,a_1,\ldots,a_t \rangle$ for some $t \geq 1$. So $f = \sum_{i=0}^\infty a_iX^i = \sum_{j=0}^t a_jf_j$, where $f_j \in \operatorname{Nil}(A) \cdot A \llbracket X \rrbracket \subseteq \operatorname{Nil}(A\llbracket X \rrbracket) \leq A\llbracket X \rrbracket$ for $j = 0,\ldots,t$. Thus, $f \in \operatorname{Nil}(A\llbracket X \rrbracket)$. 
...
\begin{definition}\label{1.70}
    Define
    \[A\llbracket X,Y \rrbracket = A\llbracket X \rrbracket \llbracket Y \rrbracket,\]
    and for $d \geq 2$,
    \[A\llbracket X_1,\ldots,X_d \rrbracket = A \llbracket X_1,\ldots,X_{d-1} \rrbracket \llbracket X_d \rrbracket.\]
\end{definition}

\begin{fact}\label{1.71}
    $A\llbracket X_1,\ldots,X_d \rrbracket = \{\sum_{\underline n \in \bbN_0^d} a_{\underline n} \underline X^{\underline n} \mid a_{\underline n} \in A\}$ for $d \geq 1$, where $\underline X^{\underline n} = X_1^{n_1} \cdots X_d^{n_d}$ and $\underline n = (n_1,\ldots,n_d) \in \bbN_0^d$.
\end{fact}
...
\begin{definition}\label{1.73}
    ...
    \begin{enumerate}
        \item $n = m$,
        \item there exists $\sigma \in S_n$ such that $\alpha_i = u_i \beta_{\sigma(i)}$ with $u_i \in R^{\times}$ for $i = 1,\ldots,n$.
    \end{enumerate}
\end{definition}

\begin{example}\label{1.74}
    Examples of UFD's: $k$, $\bbZ$, $\bbZ[i]$, $\bbZ[\frac{-1+\sqrt {-3}}{2}],\bbZ[\frac{-1+\sqrt{-19}}{2}]$, $k[X]$, $k\llbracket X \rrbracket$, $k[X_1,\ldots,X_n]$, $k\llbracket X_1,\ldots,X_n \rrbracket $ and $k\llbracket \{X_i\}_{i \in I} \rrbracket$.
\end{example}
...
\begin{example}\label{1.77}
    ...
    \begin{enumerate}
        ...
        \item
            $k[X^{2},X^{3}] = \{a_0+a_2X^{2}+a_3X^{3}+a_4X^{4}+\cdots + a_nx^{n} \mid n \geq 0, a_i \in k,\fa i = 0,2,3,\ldots,n\}$, where $X^{6} = X^{2} \cdot X^{2} \cdot X^{2} = X^{3} \cdot X^{3}$ with $X^{2}$ and $X^{3}$ irreducible here.
    \end{enumerate}
\end{example}
...
\begin{proof}
    \begin{enumerate}
        ...
        \item ``$\supseteq$''. It follows from (a). ``$\subseteq$''. Let $\ffa = I \times J \leq R_1 \times R_2$, where $I \subseteq R_1$ and $J \subseteq R_2$. Then $(r_1i,r_2j) = (r_1,r_2)(i,j) \in I \times J$ for any $(r_1,r_2) \in R_1 \times R_2$ and $(i,j) \in I \times J$, i.e., for any $r_1 \in R_1,r_2 \in R_2,i \in I,j \in J$, we have $r_1i \in I$ and $r_2j \in J$. So $I \leq R_1$ and $J \leq R_2$.
...
\begin{proof}
    \begin{enumerate}
        \item Since $(1_{R_1},1_{R_2},\cdots) \in \prod_{i=1}^\infty R_i$, but $(1_{R_1},1_{R_2},\cdots) \not \in \bigoplus_{i=1}^\infty R_i$, $\bigoplus_{i=1}^\infty R_i \subsetneq \prod_{i=1}^\infty R_i$. \\
            ...
            Suppose $\bigoplus_{i=1}^\infty R_i \subseteq R_1 \times \cdots \times R_{n-1} \times \ffm_n \times R_{n+1} \times \cdots$ for some $n \geq 1$ and $\ffm_n \in \operatorname{Jac}(R_n)$. Consider $(1,\ldots,\mathpunct{\underset{\mathclap{\substack{\ n^{\text{th}} \\ \textstyle }}}{1}},0,0,\cdots) \in \bigoplus_{i=1}^\infty R_i$. Since $1 \not \in \ffm$, $(1,\ldots,\mathpunct{\underset{\mathclap{\substack{\ n^{\text{th}} \\ \textstyle }}}{1}},0,0,\cdots) \not \in R_1 \times \cdots \times R_{n-1} \times \ffm_n \times R_{n+1} \times \cdots$, a contradiction. \qedhere
    \end{enumerate}
\end{proof}
\end{comment}

\begin{example} \label{1.85}
    We have $\bbQ[X] = \bbQ + X\bbQ[X]$. Let 
    \[R = \bbZ + X \bbQ[X] = \{n + q_1x + \cdots + q_kX^{n} \mid n \in \bbZ, k \geq 1, q_i \in \bbQ,\fa i = 1,\ldots,k\}.\]
...
\begin{example}\label{1.86}
    Let $R = k[X_1,X_2,X_3\cdots,\frac{X_1}{X_2},\frac{X_1}{X_2^{2}},\frac{X_1}{X_2^{3}},\ldots,\frac{X_2}{X_3},\frac{X_2}{X_3^{2}},\ldots,\cdots,\frac{X_n}{X_{n+1}},\frac{X_n}{X_{n+1}^{2}},\cdots]$. Let $\ffm$ be the maximal ideal $(X_1,X_2,X_3,\ldots,\frac{X_n}{X_{n+1}},\frac{X_n}{X_{n+1}^{2}},\cdots)$ of $R$. Then $R_\ffm$ has no irreducible elements and is not a field.
\end{example}

\begin{example}\label{1.87}
    We have $\bbQ[X]/(X^{2}-2) \cong \bbQ(\sqrt 2)$. Let $\overbar{\bbQ} = \bbQ(\{\alpha \mid f \in \bbQ[X] \text{ irreducible}, f(\alpha) = 0\})$, i.e., $\overbar{\bbQ}$ is the algebraic closure of $\bbQ$. Let $\overbar{\bbZ} = \{\alpha \in \overbar{\bbQ} \mid g \in \bbZ[X] \text{ monic},g(\alpha) = 0\}$. Let $\alpha \in \overbar{Z}$ and suppose $\alpha$ is irreducible. Then there exists $n \geq 1$ and $a_i \in \bbZ$ for $i=0,\ldots,n-1$ such that $\alpha$ is a root of $x^{n} + a_{n-1}x^{n-1} + \cdots + a_0$. Consider $x^{2n} + a_{n-1}x^{2n-2} + \cdots + a_0$ has a root $\beta \in \overbar{\bbZ}$ such that $\beta^{2} = \alpha$.
\end{example}
...
\begin{example}\label{1.99}
    Consider factorization only in a monoid $\langle 0,2,3,4,5,6 \rangle$, $2,3$ are irreducibles. $\langle \frac{1}{2 \cdot 3}, \frac{1}{2^{2} \cdot 5}, \frac{1}{2^{3} \cdot 7},\ldots,\frac{1}{2^{n}p_n}, \cdots \rangle$ is atomic but not ACCP.
\end{example}
