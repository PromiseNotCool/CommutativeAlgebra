...
\begin{proposition}
    ...
    \begin{enumerate}
        ...
        \item $\ffa \leq A$ is prime if and only if $\ffa \llbracket X \rrbracket \leq A\llbracket X \rrbracket$ is prime if and only if $\langle \ffa,X \rangle A \llbracket X \rrbracket \leq A\llbracket X \rrbracket$ is prime. \par 
            Let $\epsilon: A \xrightarrow {\subseteq} A\llbracket X \rrbracket$. Then $\epsilon^*: \operatorname{Spec}(A\llbracket X \rrbracket) \to \operatorname{Spec}(A)$ taking $\ffp \llbracket X \rrbracket$ to $\ker(p \circ \epsilon)$ is always onto and never 1-1, where $p$ is the natural projection from $A\llbracket X \rrbracket$ to $A\llbracket X \rrbracket/\ffp\llbracket X \rrbracket$. 
        \item $\ffa \leq A$ is maximal if and only if $\langle \ffa,X \rangle A \llbracket X \rrbracket \leq A \llbracket X \rrbracket$ is maximal. Also, $\ffa \llbracket X \rrbracket A\llbracket X \rrbracket$ is never maximal.
        \item Let $\operatorname{m-Spec}(A\llbracket X \rrbracket) = \{\text{maximal ideals of}A\llbracket X \rrbracket\} \subseteq \operatorname{Spec}(A\llbracket X \rrbracket)$. Let $\ffm \in \operatorname{m-Spec}(A\llbracket X \rrbracket)$. Then 
            ...
            Therefore, 
            \begin{align*}
                \operatorname{m-Spec}(A) &\leftrightarrows \operatorname{m_Spec}(A\llbracket X \rrbracket) \\
...
\begin{proof}
    \begin{enumerate}
        ...
        \item 
            ...
            \begin{center}
                \begin{tikzcd}
                    A\llbracket X \rrbracket \arrow[r,"\pi"]  \arrow[d,"\tau \llbracket X \rrbracket"'] & A \arrow[d,"\tau"] \\
        ...
        \item 
            ...
            Let $\ffp \in \operatorname{Spec}(A)$. By the proof of (c) and (d), $\ffp \llbracket X \rrbracket \cap A = \ffp$ and $\langle \ffp,A \rangle A\llbracket X \rrbracket \cap A = \ffp$. So $\ffp\llbracket X \rrbracket, \langle \ffp,X \rangle A\llbracket X \rrbracket \in \operatorname{Spec}(A\llbracket X \rrbracket)$ and 
            \begin{align*}
                \epsilon^*(\ffp \llbracket X \rrbracket) &= \ker(p \circ \epsilon) = \epsilon^{-1}(\ffp \llbracket X \rrbracket) = \ffp \llbracket X \rrbracket \cap A = \ffp \\
                                                         &= \langle \ffp,A \rangle A\llbracket X \rrbracket \cap A = \epsilon^{-1}(\langle \ffp,X \rangle A\llbracket X \rrbracket) = \ker(p \circ \epsilon) = \epsilon^*(\langle \ffp,X \rangle A\llbracket X \rrbracket). 
            \end{align*}
            So $\epsilon^*$ is onto. Also, since $X \not \in \ffp \llbracket X \rrbracket$, but $X \in \langle \ffp,X \rangle \llbracket X \rrbracket$, we have $\epsilon^*$ is not 1-1.
        \item 
        ...
        \item 
            \begin{enumerate}
                \item [(2)]
                    Since $X \in \operatorname{Jac}(A\llbracket X \rrbracket)$ by (f), we have $X \in \ffm$ for any $\ffm \in \operatorname{m-Spec}(A\llbracket X \rrbracket)$. Prime correspondence under quotients.
                \item [(3)] Since $\langle \ffn,X \rangle \subseteq \ffm$, we have $\langle \ffn,X \rangle \subseteq \ffm$. Note by (i) and (1), $\langle \ffn,X \rangle \leq A \llbracket X \rrbracket$ are maximal. Also, since $\ffm \leq A\llbracket X \rrbracket$ is maximal, $\langle \ffn,X \rangle = \ffm$. \qedhere
            \end{enumerate} 
        \qedhere
    \end{enumerate}
\end{proof}
